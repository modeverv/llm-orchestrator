...
- [ ] `_command_from_text()` のマッチ条件を絞り込む（例：`$ ` 行のみに限定し、テキスト推測を無効化）
- [ ] または `_command_from_text()` を完全に無効化し、tool call 経由の構造化データだけを採用する
- [ ] 誤検知が起きにくいことをテストで明示する（誤マッチしない入力パターンを追加）
...
- [ ] `tests/test_summarizer.py` に `_commands_from_events()` の直接テストを追加する
...
### P5-C: ClaudeWorker / CodexWorker の `Commands Run` 空問題（将来対応）

P4-B で B案（GeminiWorker のみ）を採用したため、Claude / Codex ジョブの
`## Commands Run` は引き続き空になる。
Claude CLI の `--print` 出力には tool_use ブロックが JSON で含まれており、
同様の抽出ロジックを追加できる余地がある。

- [ ] ClaudeWorker の stdout ストリームから `tool_use` ブロック（`{"type":"tool_use","name":"...","input":{...}}`）
  を検出し、`{"event_type":"command","command":"..."}` を events.jsonl に追記する
- [ ] CodexWorker の `--json` 出力からコマンド実行イベントを抽出する方法を調査する

### P5-D: `test_summarizer.py` のテスト拡充

`summarizer.py` は 247 行あるが、直接テストされていない関数が多い。

- [ ] `_extract_decisions_and_next_action()` のテストを追加する
  - セクションヘッダーあり（`## Decisions Made` / `## 変更内容` 等）の場合に抽出できること
  - セクションヘッダーなしの場合に first-line フォールバックが使われること
  - 空メッセージの場合に空リストを返すこと
- [ ] `_verification_lines()` のテストを追加する
  - `verify_outputs=None` と `verify_outputs=[]` で挙動が異なることを確認
- [ ] `_commands_from_events()` のテストを追加する（P5-B と共通）
...
- [ ] `_iter_tool_calls(value, depth=0, max_depth=5)` のように深さ上限を追加する
- [ ] 深さ超過時はそれ以上の走査を打ち切る
